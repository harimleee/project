...
$(function () {
  ...
  $('aside').load('common_inc.html aside p', top); //end       
  ...
  function foot() {
    $('.footer_right .top div').eq(0).find('a').on('click', function (e) {
      ...
      location.href = $(this).attr('href'); //value값과 .tab a text 값이 같으면 location href로 이동
    });
  }
  ...
  function head() {
    ...
    $('.popup .first div div').eq(0).find('a').on('click', function (e) {
      ...
      setTimeout(function () {
        $('.popup .second').css('transform', 'translateX(0%)');
      }, 50);
    });
