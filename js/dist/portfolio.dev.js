...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var graph = {
    y: $('#graph').offset().top - $(window).height() / 2.7,
    state: true
  };
  $(window).on('scroll', function () {
  ...
  function bubble() {
    ...
    function drawCircle(y) {
      this.x = Math.random() * cw, this.y = Math.random() * ch + 300, this.speed = Math.random() * 5 + 4, this.r = Math.random() * 60 + 30;
    }
  ...
  if (mql.matches) {
    var _wheelDelta = function _wheelDelta() {
      return prevScrollTop - nowScrollTop > 0 ? 'up' : 'down';
    };
    ...
    var _bubble = function _bubble() {
      ...
      function drawCircle(y) {
        this.x = Math.random() * cw, this.y = Math.random() * ch + 300, this.speed = Math.random() * 5 + 3, this.r = Math.random() * 40 + 20;
      }
